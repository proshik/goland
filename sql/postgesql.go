...
func main() {
	postgres := NewPostgres("postgres://postgres:password@localhost:5432/githubstatbot?sslmode=disable")
	//err := postgres.Add(123123, "test")
	//if err != nil {
	//	fmt.Print(err)
	//}

	accessToken, err := postgres.Get(123123)
	if err != nil {
		fmt.Println(err)
	}
	fmt.Printf("accessToken=%s", accessToken)

	//err = postgres.Delete(123123)
	//if err != nil {
	//	fmt.Println(err)
	//}
}
